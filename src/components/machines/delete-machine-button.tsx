...
export function DeleteMachineButton({ id, name, variant = "ghost" }: { id: string; name: string; variant?: "ghost" | "outline" | "danger" }) {
  const [pending, start] = useTransition();
  const router = useRouter();
  const [error, setError] = useState<string | null>(null);

  const onDelete = () => {
    const msg = "Delete \"" + name + "\"? This is permanent if the machine has no history. If it has history it will be retired instead. Active assignments/sessions block deletion.";
  ...
  return (
    <span className="inline-flex flex-col gap-1">
      <Button
        ...
        disabled={pending}
        onClick={onDelete}
        title={variant === "ghost" ? "Delete machine" : undefined}
